...
//-----------------------------------------------------------------------------
void Usb_Resume ();                // resumes USB operation
void Usb_Reset ();                 // Called after USB bus reset
void Handle_Control ();            // Handle SETUP packet on EP 0
void Handle_In1 ();                // Handle in packet on EP 1
void Handle_Out1 ();               // Handle out packet on EP 1
void Usb_Suspend ();               // This routine called when
...
//
//-----------------------------------------------------------------------------
void Usb_ISR () interrupt 8 {      // Top-level USB ISR
      ...
      if (!(ControlReg & rbINPRDY) )   // Don't overwrite last packet
      {
         ...
         if ((!(ControlReg & rbSUEND)) || (!(ControlReg & rbOPRDY)))
         {
            ...
            if (DATASIZE >= EP0_PACKET_SIZE)
            {
               ...
               Fifo_Write (FIFO_EP0, EP0_PACKET_SIZE, DATAPTR);
            ...
            else
            {
               ...
               Fifo_Write (FIFO_EP0, DATASIZE, DATAPTR);
   ...
   if (EP_STATUS[0] == EP_RX)          // See if endpoint should transmit
   {
      ...
      if (ControlReg & rbOPRDY)        // Verify packet was received
      {
         ControlReg = rbSOPRDY;
   		if (DATASIZE >= EP0_PACKET_SIZE)
   		{
            Fifo_Read(FIFO_EP0, EP0_PACKET_SIZE, DATAPTR);
            // Advance data pointer
            DATAPTR  += EP0_PACKET_SIZE;
            // Decrement data size
            DATASIZE -= EP0_PACKET_SIZE;
   		...
   		else
   		{
            ...
            Fifo_Read (FIFO_EP0, DATASIZE,  DATAPTR);
         ...
         // call the OUT_Report function and pass the Report
...
//
// Fifo_Write is used for function calls made in the foreground routines,
// and Fifo_Write is used for calls made in an ISR.
...
void SendPacket () {
    ...
    if (EP_STATUS[1] == EP_HALT) {      // If endpoint is currently halted, send a stall
        ...
        POLL_WRITE_BYTE (EINCSR1, rbInINPRDY);  // Set In Packet ready bit,
